...
The historical bug this fixes: previously, `gen` was computed as **depth from roots** (max distance to any childless ancestor + 1). When one branch had more recorded generations than the other (e.g. Pazmiño family has 4 generations recorded, Jácome family only 2), the parent and their siblings ended up on different rows. Pass 2 alignment dragged the parent down, but their siblings stayed shallow.
