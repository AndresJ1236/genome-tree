...
2. **Update `.env.production`** on the server:
   ```bash
   ssh -i C:/Users/andre/.ssh/truenas_key root@192.168.100.58
   nano "/mnt/The Vault/Tresure/Genome/.env.production"
   # Change SESSION_SECRET="..." to the new value
   ```

3. **Restart the app container** (no rebuild needed):
   ```bash
   cd "/mnt/The Vault/Tresure/Genome"
