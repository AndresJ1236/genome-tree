...
**Genome Tree** is a self-hosted family genealogy web application. One PostgreSQL database, one Docker stack, one extended family using it. Currently deployed for the Jácome–Pazmiño family (the user, Andrés Jácome, is the primary admin and developer).
...
| URL | https://arbol.adastranium.com |
| Host | TrueNAS SCALE Electric Eel 24.10+ at `192.168.100.58` |
...
The primary user/developer is **Andrés Jácome** (`AJ` in tree screenshots). When the user says "mi papá", they mean Wilson Jácome. "Mi mamá" = Marianella Pazmiño. The tree they see is centered on themselves (focus = AJ).

When working on layout/visibility issues, this matters because:
- The Jácome side has fewer recorded relatives (~7 people)
- The Pazmiño side has many more (~35-40 people)
