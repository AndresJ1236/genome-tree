...
| Public URL | `https://arbol.adastranium.com` |
| Public exposure | Cloudflare Tunnel (no port forwarding) |
| Deploy path on host | `/mnt/vault/Tresure/Genome` (vault is a symlink to `/mnt/The Vault`) |
| File transfer | SMB share `\\<TRUENAS_HOST>\Tresure\Genome\` accessible from Windows |
| SSH user | `root` |
| SSH key | `<USER_LOCAL_PATH>\.ssh\truenas_key` (passphrase-less, on the dev machine only) |
...
Make changes in the source tree at `C:\Users\andre\OneDrive\Estudio\USFQ\genome-tree`.
...
```powershell
robocopy "C:\Users\andre\OneDrive\Estudio\USFQ\genome-tree" `
...
```powershell
robocopy "C:\Users\andre\OneDrive\Estudio\USFQ\genome-tree\src\lib" `
...
ssh -i <SSH_KEY_PATH> root@<TRUENAS_HOST> \
  "cd /mnt/vault/Tresure/Genome && docker compose up -d --build"
```
...
Hard refresh (`Ctrl+Shift+R`) on `https://arbol.adastranium.com`. Cloudflare aggressively caches the JS bundle.
...
```bash
ssh -i <SSH_KEY_PATH> root@<TRUENAS_HOST> \
  'DB_URL=$(grep DATABASE_URL /mnt/vault/Tresure/Genome/.env.production | cut -d= -f2- | tr -d "\"")
...
| SMB path with spaces fails | `/mnt/The Vault` has a space | Already handled — symlink `/mnt/vault → /mnt/The Vault` exists on the NAS |
...
```bash
ssh -i <SSH_KEY_PATH> root@<TRUENAS_HOST> \
  "docker exec genome-db-1 pg_dump -U genome_tree genome_tree > /mnt/vault/backups/genome-$(date +%F).sql"
```
