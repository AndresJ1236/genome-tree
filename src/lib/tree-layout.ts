...
export function computeTreeLayout(
  ...
  // than the other. With BFS from focus, an uncle is always exactly one row
  // above their nephew regardless of how deep the other family branch goes.
  //
  // We still need depth-from-roots for findAutoFocus when no explicit focus
  // is provided — compute it once into a temp map.
  ...
  // Normalize the BFS-reached people first: shift so their MIN gen is 0.
  // (The focus may be far below the oldest ancestor; we want ancestors at
  // y=0 and focus at y=maxGen*GEN_H.)
  if (gen.size > 0) {
  ...
  // Disconnected people (truly no path to focus) land at gen 0 — top of the
  // canvas, alongside the oldest connected ancestors. NOT at a huge offset:
  // that would push the entire connected family thousands of rows down and
  // off-screen.
  for (const p of persons) {
  ...
  // ── Score computation ─────────────────────────────────────────────────────
  // focusId was already determined above (before the BFS-from-focus generation
  // assignment, since it drives that BFS). Reuse it for the lateral scores.
