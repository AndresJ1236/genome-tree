...
| Host        | `NAS_HOST`                            |
| Usuario     | `root`                                      |
| Llave       | `USER_HOME\.ssh\SSH_KEY`           |
| Passphrase  | `andres`                                    |
| Ruta proyecto | `NAS_DEPLOY_PATH`           |
...
$key = "$env:USERPROFILE\.ssh\SSH_KEY"
```

Prueba de conexión:
```powershell
& $ssh -i $key -o StrictHostKeyChecking=no root@NAS_HOST "echo ok"
```
...
& $scp -i $key -o StrictHostKeyChecking=no `
  ...
  'root@NAS_HOST:"NAS_DEPLOY_PATH/ruta/destino/archivo.ts"'
```
...
```powershell
& $ssh -i $key -o StrictHostKeyChecking=no root@NAS_HOST `
  "cd 'NAS_DEPLOY_PATH' && docker compose up -d --build app 2>&1"
```
...
```
TrueNAS SCALE (NAS_HOST)
└── Docker Compose: NAS_DEPLOY_PATH/
